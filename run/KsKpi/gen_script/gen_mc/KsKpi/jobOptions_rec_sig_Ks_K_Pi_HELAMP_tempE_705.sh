...
echo "./jobOptions_rec_sig_Ks_K_Pi_HELAMP_${ENERGYPOINT}.sh [NUM1] [NUM2]"
echo "[NUM1]: the minimum number range of job generated"
echo "[NUM2]: the maximum number range of job generated"

SEED=123
 
DIR_NAME_RTRAW="/besfs/groups/cal/dedx/$USER/bes/KsKpi/run/KsKpi/rtraw/mc/KsKpi/$ENERGYPOINT/"
DIR_NAME_DST="/besfs/groups/cal/dedx/$USER/bes/KsKpi/run/KsKpi/dst/mc/KsKpi/$ENERGYPOINT/"
mkdir -p $DIR_NAME_DST
EVENT_NO=-1

JOB_NAME="jobOptions_rec_sig_Ks_K_Pi_HELAMP_$ENERGYPOINT"
INPUT_NAME="Sig_Ks_K_Pi_HELAMP_$ENERGYPOINT"
OUTPUT_NAME="Sig_Ks_K_Pi_HELAMP_$ENERGYPOINT"
...
do
    ...
    echo "" >> $REC_NAME
    echo "RootWriter.ItemList += {\"/Event/MC/MdcMcHitCol#1\", \"/Event/MC/EmcMcHitCol#1\"};" >> $REC_NAME
    echo "" >> $REC_NAME
